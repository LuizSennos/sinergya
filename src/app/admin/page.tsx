...
// apiGetPatients: usado na aba Pacientes (só nome, vínculo, status, data)
// e nos selects dos modais. Sem dados clínicos (sem specialties, sem is_minor).
...
export default function AdminPage() {
  ...
  const [patients, setPatients] = useState<any[]>([]); // só para modais
  ...
  return (
    <main className="min-h-screen" style={{ background: "radial-gradient(ellipse at 60% 0%, rgba(30,140,104,0.07) 0%, transparent 50%), radial-gradient(circle at 10% 80%, rgba(42,127,196,0.06) 0%, transparent 40%), #f7fbf9" }}>
      ...
      <div className="px-4 md:px-8 py-6 max-w-6xl mx-auto">
        ...
        {!dataLoading && activeTab === "stats" && stats && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Usuários ativos",    value: stats.active_users,     color: "#1e8c68", icon: "✅" },
                { label: "Total usuários",     value: stats.total_users,      color: "#2a7fc4", icon: "👥" },
                { label: "Tarefas criadas",    value: stats.total_tasks,      color: "#c9a227", icon: "✅" },
                { label: "Logs de auditoria",  value: stats.total_audit_logs, color: "#7c3aed", icon: "📋" },
              ].map(s => (
        ...
        {/* PATIENTS — só nome, vínculo, status e data (sem dados clínicos) */}
          ...
          <>
            <div className="hidden md:block bg-white rounded-2xl border border-slate-100 overflow-hidden" style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
              <table className="w-full text-sm">
                <thead style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
                  <tr>
                    {["Nome", "Usuário vinculado", "Status", "Cadastro"].map(h => (
                      <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {patients.length === 0 && (
                    <tr><td colSpan={4} className="px-4 py-10 text-center text-slate-300 text-xs">Nenhum paciente cadastrado.</td></tr>
                  )}
                  {patients.map((p: any) => {
