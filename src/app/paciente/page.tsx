...
import {
  apiGetPatients, apiGetMessages, apiSendMessage,
  apiGetDiary, apiCreateDiaryEntry, apiGetTasks, apiMarkTaskDone
} from "@/lib/api";
...
export default function PacientePage() {
  ...
  useEffect(() => {
    apiGetPatients().then((ps) => {
      if (ps.length > 0) setPatient(ps[0]);
          ...
          <div className="space-y-4">
            ...
            {messages.map((msg: any) => (
              <div key={msg.id} className="bg-white rounded-xl p-5 border-l-4 border-sinergya-blue shadow-sm">
                ...
                <span className="mt-2 block text-xs text-slate-400">{formatDate(msg.created_at)}</span>
              </div>
            ))}
          ...
          <div className="space-y-3">
            ...
            {tasks.map((task: any) => (
              <div key={task.id} className={`rounded-xl p-5 border transition-all ${
                task.is_done ? "bg-slate-50 border-slate-100 opacity-70" : "bg-white border-slate-200 shadow-sm"
              }`}>
